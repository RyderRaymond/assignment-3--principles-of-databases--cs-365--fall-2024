...
function update($update_attribute, $new_value, $query_attribute, $pattern) {
    try {
        ...
        */
        ...
        $statement = null;
        echo "<h2>Entries Updated: {$count}</h2>";
    } catch( PDOException $e ) {
...
/*
The next three functions are for creating a new user, website, or for creating a new registers_at tuple for a user and website.
*/
...
function insert_user($first_name, $last_name) {
    try {
        ...
        $succeeded = ($statement -> rowCount() > 0) ? "New User Added" : "Failed to Add New User";
        $statement = null;
        echo "<h2>{$succeeded}</h2>";
...
function insert_website($website_name, $url) {
    try {
        ...
        $succeeded = ($statement -> rowCount() > 0) ? "New Website Added" : "Failed to Add Website User";
        $statement = null;
        echo "<h2>{$succeeded}</h2>";
...
function insert_account($site_id, $user_id, $username, $password, $email_address, $comment) {
    try {
        ...
        $succeeded = ($statement -> rowCount() > 0) ? "New Account Added" : "Failed to Add New Account";
        $statement = null;
        echo "<h2>{$succeeded}</h2>";
...
function delete($table, $query_attribute, $pattern) {
    try {
        ...
        if ("users" === $table && ("first_name" === $query_attribute || "last_name" === $query_attribute || "users.user_id" === $query_attribute)) {
            $query_string = "DELETE FROM {$table} WHERE {$query_attribute} = \"{$pattern}\";";
        }
        else if ("websites" === $table && ("site_name" === $query_attribute || "url" === $query_attribute || "websites.site_id" === $query_attribute)) {
            $query_string = "DELETE FROM {$table} WHERE {$query_attribute} = \"{$pattern}\";";
        }
        ...
        $count = $statement -> rowCount();
        $statement = null;
        echo "<h2>Entries Deleted: {$count}</h2>";
    } catch( PDOException $e ) {
        echo '<p>The following message was generated by function <code>update</code>:</p>' . "\n";
        echo '<p id="error">' . $e -> getMessage() . '</p>' . "\n";
